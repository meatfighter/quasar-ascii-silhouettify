...
const RGBS =
  'DAwMxQ8fE6EOwZwAADfaiBeYOpbdzMzMdnZ250hWFsYM+fGlO3j/tACeYdbW8vLyAAAAAABfAACHAACvAADXAAD/AF8AAF9fAF+HAF+' +
...
let htmlColors: string[] | undefined;

function ensureColorsInitialized() {
  if (!htmlColors) initColors();
}

export function initColors() {
  if (htmlColors) return;
...
export function findClosestColorIndexAmong(
  indices: number[],
  darkness: number,
  r: number,
  g: number,
  ...
  // use unsigned 32-bit so keys are stable even when top bit is set
  const key = (((r << 24) | (g << 16) | (b << 8) | a) >>> 0);
  const cached = closestColorCache.get(key);
  if (cached !== undefined) return cached;

  let index = 0;

  const c = chroma(r, g, b).lab(); // [L, a, b]
  c[0] *= a / 255;
  ...
  if (c[0] >= darkness) {
    ...
    switch (pal) {
      ...
      case Palette.EXTENDED_240:
        i = 255;
        minIndex = 16; // do not compare against standard 16 ANSI colors
        break;
      default:
        i = 255;
        minIndex = 0;
